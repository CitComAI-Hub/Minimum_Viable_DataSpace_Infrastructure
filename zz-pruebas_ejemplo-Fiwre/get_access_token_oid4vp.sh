...
holder_did=$(cat $4/did.json | jq '.id' -r)
...
signature=$(echo -n "${jwt_header}.${payload}" | openssl dgst -sha256 -binary -sign $4/private-key.pem | base64 -w0 | sed s/\+/-/g | sed 's/\//_/g' | sed -E s/=+$//)
