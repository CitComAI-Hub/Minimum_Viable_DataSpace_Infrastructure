...
# Define las líneas que quieres añadir
LINEAS=(
    "######"
...
for LINEA in "${LINEAS[@]}"; do
    ...
    if ! grep -Fxq "$LINEA" /etc/hosts; then
        # Si no existe, añade la línea al final del archivo
...
echo -e "$(formatear_negrita_azul "[Trust Anchor] - List of issuers: ") \
    $(curl -s -X GET http://${TIR_OPERATOR_DNS}/v4/issuers | jq)\n"

# # Register new issuer:
# echo -e "$(formatear_negrita_azul "[Trust Anchor] - Register new issuer. ") \
#     $(curl -s -X POST http://${TIL_OPERATOR_DNS}/issuer \
...
export ACCESS_TOKEN=$(curl -s -X POST "http://${KEYCLOAK_DNS}/${KEYCLOAK_USER_PATH}/openid-connect/token" \
    --header 'Accept: */*' \
...
echo -e "$(formatear_negrita_azul "[CONSUMER] - Credentials issuer information:") \
    $(curl -s -X GET http://${KEYCLOAK_DNS}/realms/test-realm/.well-known/openid-credential-issuer | jq)\n"
...
export OFFER_URI=$(curl -s -X GET "http://${KEYCLOAK_DNS}/${KEYCLOAK_USER_PATH}/oid4vc/credential-offer-uri?credential_configuration_id=user-credential" \
...
export CREDENTIAL_ACCESS_TOKEN=$(curl -s -X POST "http://${KEYCLOAK_DNS}/${KEYCLOAK_USER_PATH}/openid-connect/token" \
    --header 'Accept: */*' \
    --header 'Content-Type: application/x-www-form-urlencoded' \
...
export VERIFIABLE_CREDENTIAL=$(curl -s -X POST "http://${KEYCLOAK_DNS}/${KEYCLOAK_USER_PATH}/oid4vc/credential" \
    --header 'Accept: */*' \
    --header 'Content-Type: application/json' \
...
curl -X 'POST' http://${PAP_PROVIDER_DNS}/policy \
...
## DATA ADDITION
##
curl -X POST http://${SCORPIO_PROVIDER_DNS}/ngsi-ld/v1/entities \
...
export TOKEN_ENDPOINT=$(curl -s -X GET "http://${GATEWAY_DNS}/.well-known/openid-configuration" | jq -r '.token_endpoint');
echo -e "$(formatear_negrita_azul "[PROVIDER] -  Token Endpoint:") $TOKEN_ENDPOINT"
curl -s -X GET "http://${GATEWAY_DNS}/.well-known/openid-configuration" | jq && echo -e "\n"
...
# Try data access
curl -X GET "http://${GATEWAY_DNS}/ngsi-ld/v1/entities/urn:ngsi-ld:EnergyReport:fms-1" \
...
export PRODUCT_SPEC_ID=$(curl -s -X 'POST' http://${TM_FORUM_API_PROVIDER_DNS}/tmf-api/productCatalogManagement/v4/productSpecification \
...
export PRODUCT_OFFERING_ID=$(curl -s -X 'POST' http://${TM_FORUM_API_PROVIDER_DNS}/tmf-api/productCatalogManagement/v4/productOffering \
     -H 'Content-Type: application/json;charset=utf-8' \
     -d "{
        \"version\": \"1.0.0\",
        \"lifecycleStatus\": \"ACTIVE\",
...
echo -e "$(formatear_negrita_azul "[TMForum API] - PRODUCT_OFFERING_ID:") $PRODUCT_SPEC_ID"

########################################
# Credentials issuance at the consumer #
########################################
export USER_CREDENTIAL=$(./zz-pruebas_ejemplo-Fiwre/get_credential_for_consumer.sh http://${KEYCLOAK_DNS} user-credential);
echo -e "$(formatear_negrita_azul "[CONSUMER] - User credential:") $USER_CREDENTIAL\n"

export OPERATOR_CREDENTIAL=$(./zz-pruebas_ejemplo-Fiwre/get_credential_for_consumer.sh http://${KEYCLOAK_DNS} operator-credential);
...
export CLUSTER_CREATION_ANY_USER=$(curl -s -X POST http://${GATEWAY_DNS}/ngsi-ld/v1/entities \
    -H 'Accept: */*' \
    -H 'Content-Type: application/json' \
