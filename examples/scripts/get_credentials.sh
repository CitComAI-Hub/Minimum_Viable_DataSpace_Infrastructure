...
source ./common/text_formater.sh
# Trust Anchor: Blue
# Consumer: Orange
# Provider: Green

certs_path="../kind_minimal_ds_local/user_certs"
...
echo -e "$(blod_blue_format "################\n# TRUST ANCHOR #\n################")\n"
source ./fiware_components/trust_anchor.sh

# # Register new issuer:
# echo -e "$(blod_blue_format "[Trust Anchor] - Register new issuer. ") \
#     $(POST_register_issuer ${CLUSTER_LOCAL_IP} ${TIL_OPERATOR_DNS} '{
#         "did": "did:key:myKey79846",
#         "credentials": []}')\n"

# Get a list of the issuers:
echo -e "$(blod_blue_format "[Trust Anchor] - List of issuers: ") \
    $(GET_issuer_list ${CLUSTER_LOCAL_IP} ${TIR_OPERATOR_DNS})\n"
...
export USER_CONSUMER_TOKEN=$(POST_user_access_token ${CLUSTER_LOCAL_IP} ${KEYCLOAK_USER_PATH} \
  ${KEYCLOAK_DNS} ${KEYCLOAK_USER_NAME} ${KEYCLOAK_USER_PASSWORD})
echo -e "$(blod_orange_format "[CONSUMER] - User access TOKEN details:") \n$(echo $USER_CONSUMER_TOKEN | jq)"
export ACCESS_TOKEN=$(echo $USER_CONSUMER_TOKEN | jq -r ".access_token")
echo -e "$(blod_orange_format " >> User access TOKEN (Keycloak):") $ACCESS_TOKEN\n"

# Get the credentials issuer information (Optional):
echo -e "$(blod_orange_format "[CONSUMER (Optional in local example)] - Credentials issuer information:") \
  \n$(GET_credential_issuer_info ${CLUSTER_LOCAL_IP} ${KEYCLOAK_DNS})\n"
...
export CREDENTIALS_OFFER_URI=$(GET_credential_offer_uri ${CLUSTER_LOCAL_IP}\
  ${KEYCLOAK_USER_PATH} ${KEYCLOAK_DNS} ${ACCESS_TOKEN})
echo -e "$(blod_orange_format "[CONSUMER] - Credencital offer URI (expiration time 30s):") \n$(echo $CREDENTIALS_OFFER_URI | jq)"
export OFFER_URI=$(echo $CREDENTIALS_OFFER_URI | jq '"\(.issuer)\(.nonce)"' -r)
echo -e "$(blod_orange_format " >> Offer URI for \e[3muser-credential\e[0m") \
$(blod_orange_format "(expiration time 30s):") $OFFER_URI\n"

# Use the offer uri to retrieve the actual offer:
export ACTUAL_OFFER=$(GET_pre_authorized_code ${OFFER_URI} \
  ${CLUSTER_LOCAL_IP} ${KEYCLOAK_DNS} ${ACCESS_TOKEN})
echo -e "$(blod_orange_format "[CONSUMER] - Actual offer:") \n$ACTUAL_OFFER"
export PRE_AUTHORIZED_CODE=$(echo $ACTUAL_OFFER | jq '.grants."urn:ietf:params:oauth:grant-type:pre-authorized_code"."pre-authorized_code"' -r)
echo -e "$(blod_orange_format " >> Pre authorized code (expiration time 30s):") $PRE_AUTHORIZED_CODE \n"

# Exchange the pre-authorized code from the offer with an AccessToken at the authorization server:
export USER_CREDENTIAL=$(POST_credential_access_token ${CLUSTER_LOCAL_IP} \
  ${KEYCLOAK_USER_PATH} ${KEYCLOAK_DNS} ${PRE_AUTHORIZED_CODE})
echo -e "$(blod_orange_format "[CONSUMER] - User credentials:") \n$USER_CREDENTIAL"
export CREDENTIAL_ACCESS_TOKEN=$(echo $USER_CREDENTIAL | jq '.access_token' -r)
echo -e "$(blod_orange_format " >> Credential Access Token:") $CREDENTIAL_ACCESS_TOKEN \n"

# Use the returned access token to get the actual credential:
export VERIFIABLE_CREDENTIAL=$(POST_verifiable_credential ${CLUSTER_LOCAL_IP} \
  ${KEYCLOAK_USER_PATH} ${KEYCLOAK_DNS} ${CREDENTIAL_ACCESS_TOKEN} '.credential')
echo -e "$(blod_orange_format "[CONSUMER] !! VERIFIABLE CREDENTIAL (format jwt_vc):") $VERIFIABLE_CREDENTIAL"
echo -e "$(blod_orange_format "** Verifiable Credential codified in jwt for by used in the data space.")\n"
...
#         "name": "M&P K8S"
#      }' | jq '.id' -r )
# echo -e "\n$(blod_blue_format "[TMForum API] - PRODUCT_SPEC_ID:") $PRODUCT_SPEC_ID"
...
#       }
#     }"| jq '.id' -r )
# echo -e "$(blod_blue_format "[TMForum API] - PRODUCT_OFFERING_ID:") $PRODUCT_OFFERING_ID\n"
...
# echo -e "$(blod_blue_format "[CONSUMER] - User credential:") $USER_CREDENTIAL\n"

# export OPERATOR_CREDENTIAL=$(./common/get_credential_for_consumer.sh ${KEYCLOAK_DNS} ${CLUSTER_LOCAL_IP} operator-credential);
# echo -e "$(blod_blue_format "[CONSUMER] - Operator credential:") $OPERATOR_CREDENTIAL\n"
...
# echo -e "$(blod_blue_format "[??] - USER ACCESS TOKEN:") $ACCESS_TOKEN_USER"
...
#       }
#     }')
# echo -e "$(blod_blue_format "[PROVIDER] - Cluster creation (any user, 403 IS OK!!):\n") $CLUSTER_CREATION_ANY_USER\n"
...
# echo -e "$(blod_blue_format "[??] - PROVIDER ACCESS TOKEN (no token will be returned, NULL IS OK!!):") $ACCESS_TOKEN_OPERATOR\n"

# export CONSUMER_DID=$(curl -s -X GET http://${DID_CONSUMER_DNS}/did-material/did.env | cut -d'=' -f2);
# echo -e "$(blod_blue_format "[CONSUMER] - Did consumer:") $CONSUMER_DID\n"
...
# echo -e "$(blod_blue_format "[PROVIDER] - OFFER_ID:") $OFFER_ID\n"
...
#          }
#        ]}" | jq '.')
# echo -e "$(blod_blue_format "[TMFORUM] - ORDER_CREATION:")\n$ORDER_CREATION\n"

# export ACCESS_TOKEN_OPERATOR=$(./zz-pruebas_ejemplo-Fiwre/get_access_token_oid4vp.sh http://${GATEWAY_DNS} $OPERATOR_CREDENTIAL operator $certs_path);
# echo -e "$(blod_blue_format "[??] - OPERATOR ACCESS TOKEN:") $ACCESS_TOKEN_OPERATOR\n"
