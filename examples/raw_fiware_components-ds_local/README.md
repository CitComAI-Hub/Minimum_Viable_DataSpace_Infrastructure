...
The realm configuration is provided in the `default_config_all-realm.json` file. This file contains the necessary configuration for the Keycloak **realm**, including **clients**, **roles**, and **users**.

>[!WARNING]
>
> Before importing the realm, ensure that you changed the did:key values in the `default_config_all-realm.json` file to match the did:key generated by the did-helper. You can find the did in the `did.env` file inside the Keycloak container.
